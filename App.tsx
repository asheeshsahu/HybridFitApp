...
export default function App() {
  ...
  const setWorkoutShift = useCallback(async (shift: WorkoutShift) => {
    ...
    const restDay = getWorkoutForDay(new Date().getDay(), scheduleOffset).type === 'rest';
    const active = restDay ? (shift === 'evening' ? restDayRemindersEvening : restDayRemindersMorning) : (shift === 'evening' ? eveningReminders : reminders);
    await scheduleAllReminders(active);
  }, [scheduleOffset, reminders, remindersEnabled]);
  ...
  useEffect(() => {
    const checkAutoSwitch = async () => {
      ...
      if (remindersEnabled) {
        const restDay = getWorkoutForDay(new Date().getDay(), scheduleOffset).type === 'rest';
        const active = restDay ? restDayRemindersEvening : eveningReminders;
        await scheduleAllReminders(active);
      }
    };
    checkAutoSwitch();
  }, [isRestDay, scheduleOffset, remindersEnabled, completionData.completions['workout'], completionData.completions['recovery_walk'], completionData.completions['recovery_stretch']]);
  ...
  const loadData = async () => {
    try {
      const today = getTodayKey();
      const [savedDays, savedReminders, savedHydration, savedCompletions, savedMacros, savedShift, savedOffset, savedRemindersEnabled] = await Promise.all([
      ...
      let remindersToUse = defaultReminders;
      let shift: WorkoutShift = 'morning';
      let offset = 0;
      ...
      if (savedShift === 'morning' || savedShift === 'evening') {
        shift = savedShift;
        setWorkoutShiftState(savedShift);
      }

      const parsedOffset = savedOffset ? parseInt(savedOffset, 10) : 0;
      if (!isNaN(parsedOffset) && parsedOffset >= 0) {
        offset = parsedOffset;
        setScheduleOffset(parsedOffset);
      }

      const enabled = savedRemindersEnabled !== 'false';
      setRemindersEnabled(enabled);

      const restDay = getWorkoutForDay(new Date().getDay(), offset).type === 'rest';
      const activeRemindersToSchedule = restDay
        ? (shift === 'evening' ? restDayRemindersEvening : restDayRemindersMorning)
        : (shift === 'evening' ? eveningReminders : remindersToUse);
      if (enabled) {
        await scheduleAllReminders(activeRemindersToSchedule);
      } else {
  ...
  const scheduleAllReminders = async (reminderList: Reminder[]) => {
    if (!Device.isDevice) return;
    await Notifications.cancelAllScheduledNotificationsAsync();
    for (const reminder of reminderList.filter(r => r.enabled)) {
  ...
  const handleRescheduleYes = useCallback(async () => {
    ...
    if (remindersEnabled) {
      const restDay = getWorkoutForDay(new Date().getDay(), newOffset).type === 'rest';
      const active = restDay ? (workoutShift === 'evening' ? restDayRemindersEvening : restDayRemindersMorning) : (workoutShift === 'evening' ? eveningReminders : reminders);
      await scheduleAllReminders(active);
    }
    setShowEndOfDayModal(false);
    Alert.alert('✓ Rescheduled', 'Your workout has been moved to tomorrow.');
  }, [workoutShift, reminders, remindersEnabled]);
      ...
      <SafeAreaView style={styles.container}>
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          <Header
            ...
            onTurnOn={async () => {
              await scheduleAllReminders(activeReminders);
